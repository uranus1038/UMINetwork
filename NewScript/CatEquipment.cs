...
public class CatEquipment : MonoBehaviour
{
		...
		private void EquipArmor(string nArmor)
	{
		...
		skinnedMeshRenderer.sharedMesh = CatEquipment.getEquipArmorMash(nArmor);
		skinnedMeshRenderer.material = CatEquipment.getEquipArmorMaterial(nArmor);
	...
	private void EquipHelm(string nHelm)
	{
		GameObject helm_1 = getEquipHelm(nHelm);
		this.gameObject_1 = (GameObject)UnityEngine.Object.Instantiate(helm_1, Vector3.zero, Quaternion.identity);
		this.gameObject_1.transform.parent = helm_0.transform;
		this.gameObject_1.transform.localPosition = Vector3.zero;
		this.gameObject_1.transform.localRotation = Quaternion.Euler(0f, 270f, 90f);
	...
	private static Material getEquipArmorMaterial(string nArmorMaterial)
	{
		...
		Texture2D texture2D = (Texture2D)Resources.Load("GameAssets/Characters/Heroes/Cat/Armors/Overlay/Cat1", typeof(Texture2D));
		Color[] pixels = texture2D.GetPixels(0);
		switch (nArmorMaterial)
		{
			case "a_all1":
				texture2D2 = (Texture2D)Resources.Load("GameAssets/Characters/Heroes/Cat/Armors/Materials/Cat_scout1", typeof(Texture2D));
				break;
			default:
				texture2D2 = (Texture2D)Resources.Load("GameAssets/Characters/Heroes/Cat/Armors/Materials/Cat_nude1", typeof(Texture2D));
				break;
		}
	...
	private static Mesh getEquipArmorMash(string nArmorMash)
	{
		...
		{
			case "a_all1":
				nArmor_0 = (GameObject)Resources.Load("GameAssets/Characters/Heroes/Cat/Armors/Cat_scout", typeof(GameObject));
				break;
			default:
				nArmor_0 = (GameObject)Resources.Load("GameAssets/Characters/Heroes/Cat/Armors/Cat_nude", typeof(GameObject));
				break;
		}
	...
	private static GameObject getEquipHelm(string nHelm)
	{
		GameObject result;
		switch (nHelm)
		{
